...
### v1.2.4（最小通知间隔调整为 2 秒）

**用户反馈**："改成如果这次通知距离上次通知间隔不到两秒就不要通知"

**改动**：

将 `sendNotification()` 中的最小通知间隔从 1 秒调整为 2 秒。距上次通知不足 2 秒时直接跳过，不发送通知。

### v1.2.3（修复后台标签页通知延迟问题）

**用户反馈**："有时候可以正常通知，有时候好像需要我从别处点回来他才给我通知"

**根本原因**：

Chrome 为了省电，会对**后台标签页**的 `setTimeout` 和 `setInterval` 进行节流：
- `setTimeout` 的最小延迟被强制提升到 1 秒以上，甚至完全暂停
- `setInterval` 的最小间隔也被提升到 1 秒以上

v1.1.9 引入的"延迟二次确认"机制用了 `setTimeout(200ms)`，在后台标签页里这个定时器会被延迟到几秒甚至更久才执行。当用户切回 z.ai 标签页时，标签页重新激活，积压的定时器立刻执行，通知才发出来——这就是"需要点回标签页才通知"的原因。

**修复**：

移除 `setTimeout` 二次确认，改为**直接调用** `confirmCompletion()`：
- 按钮从 `running` 变 `idle` 时，立即调用 `confirmCompletion`
- `confirmCompletion` 内部会再次检查按钮状态，如果又变回 `running`（瞬态闪动）就跳过
- 这样既保留了瞬态闪动防护，又避免了后台标签页的定时器节流问题

**为什么安全**：
- MutationObserver 在后台标签页是**实时触发**的（Chrome 不节流 observer 回调）
- 按钮重新出现的瞬间，observer 立即触发 `checkButtonStateChange`
- `confirmCompletion` 同步执行，不依赖任何定时器
- 500ms 轮询作为兜底（后台会被节流到 1s，但仍能捕获遗漏的状态变化）

### v1.2.2（最小通知间隔保护，1 秒内不重复通知）

**用户反馈**："如果这次通知距离上次通知间隔不到一秒就不要通知"

**改动**：

在 `sendNotification()` 方法开头加入最小通知间隔保护：
- 记录每次发送通知的时间戳 `lastNotificationTime`
- 距上次通知不足 1 秒时直接 `return`，不发送通知
- 这个保护在 `sendNotification` 内部，覆盖所有调用路径（按钮检测、文本轮询、MutationObserver）

**效果**：即使按钮瞬态抖动或多个检测路径同时触发，1 秒内最多只发一次通知。
